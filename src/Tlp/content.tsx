...
export const content: [string, string, string, string][] = [[
    '1',
    ...
    '2.0131',
    '空间对象必须处在无限的空间之中。（一个空间点就是一个主目位置。）\n视域里的一个斑块，虽然不一定是红的，但它必须有某种颜色：所以说它被颜色空间所包围。音调必须具有<emphasis>某种</emphasis>高度，触觉对象必须具有<emphasis>某种</emphasis>硬度，等等。',
...
[
    ...
    String.raw`Beachtet man nicht, dass der Satz einen von den Tatsachen unabhängigen Sinn hat, so kann man leicht glauben, dass wahr und falsch gleichberechtigte Beziehungen von Zeichen und Bezeichnetem sind. Man könnte dann z. B. sagen, dass „<katex>p</katex>“ auf die wahre Art bezeichnet, was „<katex>\sim p</katex>“ auf die falsche Art, etc.`,
], [
    '4.062',
    String.raw`我们能否用假命题——只要我们知道它们被认为是假的——来表达自己，就像我们一直用真命题表达自己一样呢？不能！因为如果我们用一个命题来说一些事物处于一定情况，而且它们确实如此，则这个命题为真；如果我们用"<katex>p</katex>"意指"<katex>\sim p</katex>"，而且情况确如我们所指的那样，那么在新的理解下"<katex>p</katex>"为真而不为假。`,
    String.raw`Can we not make ourselves understood by means of false propositions as hitherto with true ones, so long as we know that they are meant to be false? No! For a proposition is true, if what we assert by means of it is the case; and if by "<katex>p</katex>" we mean <katex>\sim p</katex>, and what we mean is the case, then "<katex>p</katex>" in the new conception is true and not false.`,
    String.raw`Kann man sich nicht mit falschen Sätzen, wie bisher mit wahren, verständigen? Solange man nur weiss, dass sie falsch gemeint sind. Nein! Denn, wahr ist ein Satz, wenn es sich so verhält, wie wir es durch ihn sagen; und wenn wir mit „<katex>p</katex>“ <katex>\sim p</katex> meinen, und es sich so verhält wie wir es meinen, so ist „<katex>p</katex>“ in der neuen Auffassung wahr und nicht falsch.`,
], [
    '4.0621',
    ...
    String.raw`Dass aber die Zeichen „<katex>p</katex>“ und „<katex>\sim p</katex>“ das gleiche sagen <emph>können</emph>, ist wichtig. Denn es zeigt, dass dem Zeichen „<katex>\sim</katex>“ in der Wirklichkeit nichts entspricht. Dass in einem Satz die Verneinung vorkommt, ist noch kein Merkmal seines Sinnes (<katex>\sim{\sim p} = p</katex>). Die Sätze „<katex>p</katex>“ und „<katex>\sim p</katex>“ haben entgegengesetzten Sinn, aber es entspricht ihnen eine und dieselbe Wirklichkeit.`,
], [
    '4.063',
    ...
    '4.27',
    String.raw`关于 n 个事态的存在和不存在，有 <katex>K_n=\sum\limits_{ν=0}^{n} \binom{n}{ν}</katex> 种可能性。这些事态的任何一种组合都可存在而同时别的组合不存在。`,
    ...
    String.raw`Bezüglich des Bestehens und Nichtbestehens von <katex>n</katex> Sachverhalten gibt es <katex>K_{n} = \sum\limits_{\nu = 0}^n\binom{n}{\nu}</katex> Möglichkeiten. Es können alle Kombinationen der Sachverhalte bestehen, die andern nicht bestehen.`,
], [
    '4.28',
    ...
    '4.42',
    String.raw`关于一个命题与n个基本命题的真值可能性符合和不符合，有 <katex>\sum\limits_{\kappa=0}^{K_n} \binom{K_n}{\kappa}=L_n</katex> 种可能情况。`,
    String.raw`With regard to the agreement and disagreement of a proposition with the truth-possibilities of <i>n</i> elementary propositions there are <katex>\sum\limits_{\kappa=0}^{K_n} \binom{K_n}{\kappa} = L_n</katex> possibilities.`,
    String.raw`Bezüglich der Übereinstimmung und Nichtübereinstimmung eines Satzes mit den Wahrheitsmöglichkeiten von <katex>n</katex> Elementarsätzen gibt es <katex>\sum\limits_{\kappa = 0}^{K_n}\binom{K_n}{\kappa} = L_{n}</katex> Möglichkeiten.`,
], [
    '4.43',
    ...
    String.raw`„<katex>p \cdot q</katex>“ ist einer der Sätze, welche „<katex>p</katex>“ bejahen und zugleich einer der Sätze, welche „<katex>q</katex>“ bejahen. Zwei Sätze sind einander entgegengesetzt, wenn es keinen sinnvollen Satz gibt, der sie beide bejaht. Jeder Satz der einem anderen widerspricht, verneint ihn.`,
], [
    '5.13',
    ...
    String.raw`Wenn wir von <katex>p \lor q</katex> und <katex>\sim p</katex> auf <katex>q</katex> schliessen, so ist hier durch die Bezeichnungsweise die Beziehung der Satzformen von „<katex>p \lor q</katex>“ und „<katex>\sim p</katex>“ verhüllt. Schreiben wir aber z. B. statt „<katex>p \lor q</katex>“ „<katex>p | q \cdot | \cdot p | q</katex>“ und statt „<katex>\sim p</katex>“ „<katex>p | p</katex>“ (<katex>p | q</katex> = weder <katex>p</katex>, noch <katex>q</katex>), so wird der innere Zusammenhang offenbar. (Dass man aus <katex>(x) \cdot fx</katex> auf <katex>fa</katex> schliessen kann, das zeigt, dass die Allgemeinheit auch im Symbol „<katex>(x) \cdot fx</katex>“ vorhanden ist.)`,
], [
    '5.132',
    ...
    String.raw`Das allgemeine Glied einer Formenreihe <katex>a</katex>, <katex>O' a</katex>, <katex>O' O' a</katex>, <katex>\dots</katex> schreibe ich daher so: „<katex>[a, x, O' x]</katex>“. Dieser Klammerausdruck ist eine Variable. Das erste Glied des Klammerausdruckes ist der Anfang der Formenreihe, das zweite die Form eines beliebigen Gliedes <katex>x</katex> der Reihe und das dritte die Form desjenigen Gliedes der Reihe, welches auf <katex>x</katex> unmittelbar folgt.`,
], [
    '5.2523',
    ...
    '5.254',
    String.raw`运算可以消失（如在“<katex>\sim\sim p</katex>”中的否定：<katex>\sim\sim p = p</katex>）。`,
    String.raw`Operations can vanish (e.g. denial in "<katex>\sim\sim p</katex>". <katex>\sim\sim p = p</katex>).`,
    String.raw`Die Operation kann verschwinden (z. B. die Verneinung in „<katex>\sim\sim p</katex>"<katex> \sim\sim p = p</katex>).`,
], [
    '5.3',
    ...
    String.raw`Die Wahrheitsfunktionen sind keine materiellen Funktionen. Wenn man z. B. eine Bejahung durch doppelte Verneinung erzeugen kann, ist dann die Verneinung — in irgend einem Sinnin der Bejahung enthalten? Verneint „<katex>\sim{\sim p}</katex>“ <katex>\sim p</katex>, oder bejaht es <katex>p</katex>; oder beides? Der Satz „<katex>\sim{\sim p}</katex>“ handelt nicht von der Verneinung wie von einem Gegenstand; wohl aber ist die Möglichkeit der Verneinung in der Bejahung bereits präjudiziert. Und gäbe es einen Gegenstand, der „<katex>\sim</katex>“ hiesse, so müsste „<katex>\sim{\sim p}</katex>“ etwas anderes sagen als „<katex>p</katex>“. Denn der eine Satz würde dann eben von <katex>\sim</katex> handeln, der andere nicht.`,
], [
    '5.441',
    String.raw`这些表面的逻辑常项的这种消失，也发现于“<katex>\sim(\exists x) \cdot \sim fx</katex>”的情形，它与“<katex>(x) \cdot fx</katex>”的所说是一样的；或者也发生于“<katex>(\exists x) \cdot fx \cdot x = a</katex>”的情形，它与“<katex>fa</katex>”说的是一回事情。`,
    String.raw`This disappearance of the apparent logical constants also occurs if "<katex>\sim(\exists x) \cdot \sim fx</katex>" says the same as "<katex>(x) \cdot fx</katex>", or "<katex>(\exists x) \cdot fx \cdot x=a</katex>" the same as "<katex>fa</katex>".`,
    String.raw`Dieses Verschwinden der scheinbaren logischen Konstanten tritt auch ein, wenn „<katex>\sim{(\exists x) \cdot \sim fx}</katex>“ dasselbe sagt wie „<katex>(x) \cdot fx</katex>“, oder „<katex>(\exists x) \cdot fx \cdot x = a</katex>“ dasselbe wie „<katex>fa</katex>“.`,
], [
    '5.442',
    ...
    String.raw`Hat die Logik Grundbegriffe, so müssen sie von einander unabhängig sein. Ist ein Grundbegriff eingeführt, so muss er in allen Verbindungen eingeführt sein, worin er überhaupt vorkommt. Man kann ihn also nicht zuerst für <emph>eine</emph> Verbindung, dann noch einmal für eine andere einführen. Z. B.: Ist die Verneinung eingeführt, so müssen wir sie jetzt in Sätzen von der Form „<katex>\sim p</katex>“ ebenso verstehen, wie in Sätzen wie „<katex>\sim{(p \lor q)}</katex>“, „<katex>(\exists x) \cdot \sim fx</katex>“ u. a. Wir dürfen sie nicht erst für die eine Klasse von Fällen, dann für die andere einführen, denn es bliebe dann zweifelhaft, ob ihre Bedeutung in beiden Fällen die gleiche wäre und es wäre kein Grund vorhanden, in beiden Fällen dieselbe Art der Zeichenverbindung zu benützen. (Kurz, für die Einführung der Urzeichen gilt, mutatis mutandis, dasselbe, was Frege („Grundgesetze der Arithmetik“) für die Einführung von Zeichen durch Definitionen gesagt hat.)`,
], [
    '5.452',
    ...
    String.raw`Jede Wahrheitsfunktion ist ein Resultat der successiven Anwendung der Operation "<katex>( — --W)(\xi, ....)</katex>" auf Elementarsätze. Diese Operation verneint sämtliche Sätze in der rechten Klammer, und ich nenne sie die Negation dieser Sätze.`,
], [
    '5.501',
    ...
    String.raw`Ich schreibe also statt "<katex>( — --W)(\xi, ....)</katex>" "<katex>N(\bar{\xi})</katex>". <katex>N(\bar{\xi})</katex> ist die Negation sämtlicher Werte der Satzvariable <katex>\xi</katex>.`,
], [
    '5.503',
    ...
    '5.51',
    String.raw`如果 <katex>\xi</katex> 只有一个值，则 <katex>N(\bar{\xi})</katex>＝<katex>\sim p</katex>（非 <katex>p</katex>）；如果它有两个值，则 <katex>N(\bar{\xi})</katex>＝<katex>\sim p \cdot \sim q</katex>（既非 <katex>p</katex> 也非 <katex>q</katex>）。`,
    String.raw`If <katex>\xi</katex> has only one value, then <katex>N(\bar{\xi})</katex>=<katex>\sim p</katex> (not <katex>p</katex>), if it has two values then <katex>N(\bar{\xi})</katex>=<katex>\sim p \cdot \sim q</katex> (neither <katex>p</katex> nor <katex>q</katex>).`,
    String.raw`Hat <katex>\xi</katex> nur einen Wert, so ist <katex>N(\overline{\xi}) = \sim p</katex> (nicht <katex>p</katex>), hat es zwei Werte, so ist <katex>N(\overline{\xi}) = \sim p \cdot \sim{q}</katex> (weder <katex>p</katex> noch <katex>q</katex>).`,
], [
    '5.511',
    ...
    String.raw`„<katex>\sim p</katex>“ ist wahr, wenn „<katex>p</katex>“ falsch ist. Also in dem wahren Satz „<katex>\sim p</katex>“ ist „<katex>p</katex>“ ein falscher Satz. Wie kann ihn nun der Strich „<katex>\sim</katex>“ mit der Wirklichkeit zum Stimmen bringen? Das, was in „<katex>\sim p</katex>“ verneint, ist aber nicht das „<katex>\sim</katex>“, sondern dasjenige, was allen Zeichen dieser Notation, welche <katex>p</katex> verneinen, gemeinsam ist. Also die gemeinsame Regel, nach welcher „<katex>\sim p</katex>“, „<katex>\sim{\sim{\sim p}}</katex>“, „<katex>\sim p \lor \sim p</katex>“, „<katex>\sim p \cdot \sim p</katex>“, etc. etc. (ad inf.) gebildet werden. Und dies Gemeinsame spiegelt die Verneinung wider.`,
], [
    '5.513',
    ...
    String.raw`Man könnte sagen: Das Gemeinsame aller Symbole, die sowohl <katex>p</katex> als <katex>q</katex> bejahen, ist der Satz „<katex>p \cdot q</katex>“. Das Gemeinsame aller Symbole, die entweder <katex>p</katex> oder <katex>q</katex> bejahen, ist der Satz „<katex>p \lor q</katex>“. Und so kann man sagen: Zwei Sätze sind einander entgegengesetzt, wenn sie nichts miteinander gemein haben, und: Jeder Satz hat nur ein Negativ, weil es nur einen Satz gibt, der ganz ausserhalb seiner liegt. Es zeigt sich so auch in Russells Notation, dass „<katex>q : p \lor \sim p</katex>“ dasselbe sagt wie „<katex>q</katex>“; dass „<katex>p \lor \sim p</katex>“ nichts sagt.`,
], [
    '5.514',
    ...
    '5.52',
    String.raw`若 <katex>\xi</katex> 的值是函项 <katex>fx</katex> 对于所有 <katex>x</katex> 值的全部取值，则 <katex>N(\bar{\xi})</katex>＝<katex>\sim(\exists x) \cdot fx</katex>。`,
    String.raw`If the values of <katex>\xi</katex> are the total values of a function <katex>fx</katex> for all values of <katex>x</katex>, then <katex>N(\bar{\xi})</katex>=<katex>\sim(\exists x) \cdot fx</katex>.`,
    String.raw`Sind die Werte von <katex>\xi</katex> sämtliche Werte einer Funktion <katex>fx</katex> für alle Werte von <katex>x</katex>, so wird <katex>N(\overline{\xi}) = \sim{(\exists x) \cdot fx}</katex>.`,
], [
    '5.521',
    ...
    String.raw`Ich trenne den Begriff <emph>Alle</emph> von der Wahrheitsfunktion. Frege und Russell haben die Allgemeinheit in Verbindung mit dem logischen Produkt oder der logischen Summe eingeführt. So wurde es schwer, die Sätze „<katex>(\exists x) \cdot fx</katex>“ und „<katex>(x) \cdot fx</katex>“, in welchen beide Ideen beschlossen liegen, zu verstehen.`,
], [
    '5.522',
    ...
    String.raw`Man kann die Welt vollständig durch vollkommen verallgemeinerte Sätze beschreiben, das heisst also, ohne irgend einen Namen von vornherein einem bestimmten Gegenstand zuzuordnen. Um dann auf die gewöhnliche Ausdrucksweise zu kommen, muss man einfach nach einem Ausdruck „es gibt ein und nur ein <katex>x</katex>, welches <katex>\dots</katex>“ sagen: Und dies <katex>x</katex> ist <katex>a</katex>.`,
], [
    '5.5261',
    ...
    String.raw`Ein vollkommen verallgemeinerter Satz ist, wie jeder andere Satz zusammengesetzt. (Dies zeigt sich daran, dass wir in „<katex>(\exists x, \phi) \cdot \phi x</katex>“ „<katex>\phi</katex>“ und „<katex>x</katex>“ getrennt erwähnen müssen. Beide stehen unabhängig in bezeichnenden Beziehungen zur Welt, wie im unverallgemeinerten Satz.) Kennzeichen des zusammengesetzten Symbols: Es hat etwas mit <emph>anderen</emph> Symbolen gemeinsam.`,
], [
    '5.5262',
    ...
    '5.531',
    String.raw`因此我不写“<katex>f(a, b) \cdot a = b</katex>”，而写“<katex>f(a, a)</katex>”（或者“<katex>f(b, b)</katex>”）。不写“<katex>f(a, b) \cdot \sim a = b</katex>”，而写“<katex>f(a, b)</katex>”。`,
    String.raw`I write therefore not "<katex>f(a, b) \cdot a = b</katex>" but "<katex>f(a, a)</katex>" (or "<katex>f(b, b)</katex>"). And not "<katex>f(a, b) \cdot \sim a = b</katex>", but "<katex>f(a, b)</katex>".`,
    String.raw`Ich schreibe also nicht „<katex>f(a,b) \cdot a = b</katex>“, sondern „<katex>f(a,a)</katex>“ (oder „<katex>f(b,b)</katex>“). Und nicht „<katex>f(a,b) \cdot \sim a = b</katex>“, sondern „<katex>f(a,b)</katex>“.`,
], [
    '5.532',
    String.raw`以此类推：我不写“<katex>(\exists x, y) \cdot f(x, y) \cdot x = y</katex>”，而写“<katex>(\exists x) \cdot f(x, x)</katex>”；不写“<katex>(\exists x, y) \cdot f(x, y) \cdot \sim x = y</katex>”，而写“<katex>(\exists x, y) \cdot f(x, y)</katex>”。（这样，罗素的“<katex>(\exists x, y) \cdot f(x, y)</katex>”就成为：“<katex>(\exists x, y) \cdot f(x, y) \lor (\exists x) \cdot f(x, x)</katex>”。）`,
    ...
    String.raw`Und analog: Nicht „<katex>(\exists x,y) \cdot f(x,y) \cdot x = y</katex>“, sondern „<katex>(\exists x) \cdot f(x,x)</katex>“; und nicht „<katex>(\exists x,y) \cdot f(x,y) \cdot \sim x = y</katex>“, sondern „<katex>(\exists x,y) \cdot f(x,y)</katex>“. (Also statt des Russell'schen „<katex>(\exists x,y) \cdot f(x,y)</katex>“: „<katex>(\exists x,y) \cdot f(x,y) \cdot \lor \cdot (\exists x) \cdot f(x,x)</katex>“.)`,
], [
    '5.5321',
    ...
    String.raw`Statt „<katex>(x) : fx \supset x = a</katex>“ schreiben wir also z. B. „<katex>(\exists x) \cdot fx \cdot \supset \cdot fa : \sim{(\exists x,y) \cdot fx \cdot fy}</katex>“. Und der Satz „<emph>nur</emph> Ein <katex>x</katex> befriedigt <katex>f()</katex>“ lautet: „<katex>(\exists x) \cdot fx : \sim{(\exists x,y) \cdot fx \cdot fy}</katex>“.`,
], [
    '5.533',
    ...
    '5.534',
    String.raw`现在我们看到，在一个正确的概念记号系统中，像“<katex>a = a</katex>”，“<katex>a = b \cdot b = c \supset a = c</katex>”，“<katex>(x) \cdot x = x</katex>”，“<katex>(\exists x) \cdot x = a</katex>”等等伪似命题是根本不能写的。`,
    String.raw`And we see that the apparent propositions like: "<katex>a = a</katex>", "<katex>a = b \cdot b = c \supset a = c</katex>", "<katex>(x) \cdot x = x</katex>". "<katex>(\exists x) \cdot x = a</katex>", etc. cannot be written in a correct logical notation at all.`,
    String.raw`Und nun sehen wir, dass Scheinsätze wie: „<katex>a = a</katex>“, „<katex>a = b \cdot b = c \cdot \supset a = c</katex>“, „<katex>(x) \cdot x = x</katex>“, „<katex>(\exists x) \cdot x = a</katex>“, etc. sich in einer richtigen Begriffsschrift gar nicht hinschreiben lassen.`,
], [
    '5.535',
    ...
    String.raw`Es gibt gewisse Fälle, wo man in Versuchung gerät, Ausdrücke von der Form „<katex>a = a</katex>“ oder „<katex>p \supset p</katex>“ u. dgl. zu benützen. Und zwar geschieht dies, wenn man von dem Urbild: Satz, Ding, etc. reden möchte. So hat Russell in den „Principles of Mathematics“ den Unsinn „<katex>p</katex> ist ein Satz“ in Symbolen durch „<katex>p \supset p</katex>“ wiedergegeben und als Hypothese vor gewisse Sätze gestellt, damit deren Argumentstellen nur von Sätzen besetzt werden könnten. (Es ist schon darum Unsinn, die Hypothese <katex>p \supset p</katex> vor einen Satz zu stellen, um ihm Argumente der richtigen Form zu sichern, weil die Hypothese für einen Nicht-Satz als Argument nicht falsch, sondern unsinnig wird, und weil der Satz selbst durch die unrichtige Gattung von Argumenten unsinnig wird, also sich selbst ebenso gut, oder so schlecht, vor den unrechten Argumenten bewahrt, wie die zu diesem Zweck angehängte sinnlose Hypothese.)`,
], [
    '5.5352',
    String.raw`同样地人们想用“<katex>\sim(\exists x) \cdot x = x</katex>”来表达“没有<emphasis>事物</emphasis>”。但是，即使这是一个命题，如果确实“有一些事物”，但这些事物与自身不是同一的，这个命题不也同样为真吗？`,
    String.raw`Similarly it was proposed to express "There are no things" by "<katex>\sim(\exists x) \cdot x = x</katex>". But even if this were a proposition — would it not be true if indeed "There were things", but these were not identical with themselves?`,
    String.raw`Ebenso wollte man „Es gibt keine <emph>Dinge</emph>“ ausdrücken durch „<katex>\sim{(\exists x) \cdot x = x}</katex>“. Aber selbst wenn dies ein Satz wäre, — wäre er nicht auch wahr, wenn es zwar „Dinge gäbe“, aber diese nicht mit sich selbst identisch wären?`,
], [
    '5.54',
    ...
    '6',
    String.raw`真值函项的一般形式是：<centered><katex>[\bar{p}, \; \bar{\xi}, \; N(\bar{\xi})]</katex></centered>这也是命题的一般形式。`,
    String.raw`The general form of truth-function is: <centered><katex>[\bar{p}, \; \bar{\xi}, \; N(\bar{\xi})]</katex></centered>This is the general form of proposition.`,
    String.raw`Die allgemeine Form der Wahrheitsfunktion ist: <centered><katex>[\bar{p}, \bar{\xi}, N(\bar{\xi})]</katex></centered>Dies ist die allgemeine Form des Satzes.`,
], [
    '6.001',
    String.raw`它只是说明：每个命题都是连续应用运算 <katex>N(\bar{ξ})</katex> 于基本命题的结果。`,
    String.raw`This says nothing else than that every proposition is the result of successive applications of the operation <katex>N'(\bar{\xi})</katex> to the elementary propositions.`,
    String.raw`Dies sagt nichts anderes, als dass jeder Satz ein Resultat der successiven Anwendung der Operation <katex>N'(\bar{\xi})</katex> auf die Elementarsätze ist.`,
], [
    '6.002',
    ...
    '6.01',
    String.raw`因此运算 <katex>\Omega'(\bar{\eta})</katex> 的一般形式是：<centered><katex>[\bar{\xi}, N(\bar{\xi})]'(\bar{\eta})(= [\bar{\eta}, \bar{\xi}, N(\bar{\xi})])</katex></centered>这是由一个命题过渡到另一个命题的最一般的形式。`,
    String.raw`The general form of the operation <katex>\Omega'(\bar{\eta})</katex> is therefore:<centered><katex>[\bar{\xi}, N(\bar{\xi})]'(\bar{\eta})(= [\bar{\eta}, \bar{\xi}, N(\bar{\xi})])</katex></centered>This is the most general form of transition from one proposition to another.`,
    String.raw`Die allgemeine Form der Operation <katex>\Omega'(\bar{\eta})</katex> ist also:<centered><katex>[\bar{\xi}, N(\bar{\xi})]'(\bar{\eta})(= [\bar{\eta}, \bar{\xi}, N(\bar{\xi})])</katex></centered>Dies ist die allgemeinste Form des Überganges von einem Satz zum anderen.`,
], [
    '6.02',
    ...
    '6.03',
    String.raw`整数的一般形式是：<katex>[0, \bar{\xi}, \bar{\xi} + 1]</katex>。`,
    String.raw`The general form of the cardinal number is: <katex>[0, \xi, \xi + 1]</katex>.`,
    String.raw`Die allgemeine Form der ganzen Zahl ist: <katex>[0, \xi, \xi + 1]</katex>.`,
], [
    '6.031',
    ...
    '6.1201',
    String.raw`例如，命题“<katex>p</katex>”和“<katex>\sim p</katex>”在结合“<katex>\sim(p \cdot \sim p)</katex>”中构成一个重言式，这就表明它们是互相矛盾的。命题“<katex>p \supset q</katex>”、“<katex>p</katex>”、“<katex>q</katex>”在形式“<katex>(p \supset q) \cdot (p) : \supset : (q)</katex>”中互相结合起来构成一个重言式，这就表明 <katex>q</katex> 从 <katex>p</katex> 并且 <katex>p \supset q</katex> 得出来。“<katex>(x) \cdot fx : \supset : fa</katex>”是一个重言式，就表明 <katex>fa</katex> 从 <katex>(x) \cdot fx</katex> 得出来，等等。`,
    ...
    String.raw`Dass z. B. die Sätze „<katex>p</katex>“ und „<katex>\sim p</katex>“ in der Verbindung „<katex>\sim (p \cdot \sim p</katex>“ eine Tautologie ergeben, zeigt, dass sie einander widersprechen. Dass die Sätze „<katex>p \supset q</katex>“, „<katex>p</katex>“ und „<katex>q</katex>“ in der Form „<katex>(p \supset q) \cdot (p) : \supset : (q)</katex>“ miteinander verbunden eine Tautologie ergeben, zeigt, dass <katex>q</katex> aus <katex>p</katex> und <katex>p \supset q</katex> folgt. Dass „<katex>(x) \cdot fx : \supset : fa</katex>“ eine Tautologie ist, dass <katex>fa</katex> aus <katex>(x) \cdot fx</katex> folgt. etc. etc.`,
], [
    '6.1202',
    ...
    '6.1221',
    String.raw`例如，如果两个命题“<katex>p</katex>”和“<katex>q</katex>”在连接“<katex>p \supset q</katex>”中构成一个重言式，那么很清楚，<katex>q</katex> 是从 <katex>p</katex> 得出来的。例如，从这两个命题自身即可看出“<katex>q</katex>”从“<katex>p \supset q \cdot p</katex>”得出来，但我们也同样可通过如下方式表明这一点：即以“<katex>p \supset q \cdot p : \supset : q</katex>”的形式把它们连结起来，然后表明这是一个重言式。`,
